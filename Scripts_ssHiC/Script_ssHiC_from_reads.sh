...
##Script to go from paired ends reads to HiC matrices in GRAAL and cooler formats, for downstream processing with ssHiCstuff

genome_dir="/mnt/e/Genomes/" #Directory that contains a folder with the reference genome
genome="S288c_DSB_LY_Capture_artificial_v12" #reference genome. Must be provided in a folder with the same name as the fasta and index files
thread=8
...
#Provide the name of the samples. Reads must be in the form eg AD572.end1.fastq.gz and AD572.end2.fastq.gz in the fastq directory
SAMP=("AD572" "AD576" )
...
#The matrix files generated can be used for downstream processing with the Script_ssHiCstuff.sh